...
import static tech.com.commoncore.utils.RxConstant.BYTE;
import static tech.com.commoncore.utils.RxConstant.GB;
import static tech.com.commoncore.utils.RxConstant.KB;
import static tech.com.commoncore.utils.RxConstant.MB;
...
public class DataUtils {
     ...
     * @return
     */
    public static boolean isNullString(@Nullable CharSequence str) {
     ...
     * @param byteNum 字节数
     * @param unit    <ul>
     *                <li>{@link RxConstant.MemoryUnit#BYTE}: 字节</li>
     *                <li>{@link RxConstant.MemoryUnit#KB}  : 千字节</li>
     *                <li>{@link RxConstant.MemoryUnit#MB}  : 兆</li>
     *                <li>{@link RxConstant.MemoryUnit#GB}  : GB</li>
     *                </ul>
     * @return 以unit为单位的size
     */
    public static double byte2Size(long byteNum, RxConstant.MemoryUnit unit) {
     ...
     * @return 字节数
     */
    public static long size2Byte(long size, RxConstant.MemoryUnit unit) {
        if (size < 0) return -1;
        switch (unit) {
            default:
            case BYTE:
    ...
    public static String getUUid8(){
        return  RandomUtil.getRandom("abcdefghijklmnopqrstuvwxyz1234567890",8);
//        String a= RandomStringUtils.random(8, "abcdefghijklmnopqrstuvwxyz1234567890");
    }

    public static double getPercentRealValue(String percentValue){
        double realvalue = 0.00;

        if(percentValue.contains("%")){
            try {
                realvalue =  stringToDouble(percentValue.substring(0,percentValue.indexOf("%")));
            } catch (NumberFormatException e) {
                e.printStackTrace();
            }
        }else{
            realvalue =  stringToDouble(percentValue);
        }
